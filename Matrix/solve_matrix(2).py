...
def solve_intensity_matrix(light_source_intensity,random_colors):
    ...
    if random_colors is True:
        ...
        RGB_end = [N,N,N]
        RGB_val = [0.3,0.3,0.3]
        A_rho_R, A_rho_G, A_rho_B = random_absorb(RGB_start, RGB_end, RGB_val,N)
    ...
    for i in range(1):
        solution_R, solution_G, solution_B = lu_solve((LU_R, piv_R), solution_R), \
                                             lu_solve((LU_G, piv_G), solution_G), \
                                             lu_solve((LU_B, piv_B), solution_B)
